...
`Neutralino.window` 命名空间包含与当前原生窗口实例相关的方法。此命名空间的方法仅适用于 xref:configuration/modes.adoc#window[窗口] 模式。
...
将给定的 DOM 元素转换为可拖动区域。用户可以通过拖动给定的 DOM 元素来拖动原生窗口。此功能适用于与 xref:configuration/neutralino.config.json.adoc#modeswindowborderless-boolean[无边框模式]一起创建自定义窗口栏。
...
* `processArgs` String：新窗口进程的额外命令行参数。从 xref:cli/internal-cli-arguments.adoc[这里] 查看所有支持的内部命令行参数。
